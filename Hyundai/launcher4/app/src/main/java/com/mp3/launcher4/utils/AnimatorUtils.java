...
import com.mp3.launcher4.holders.CategoryVideoHolder;
...
public class AnimatorUtils {
    ...
    public static AnimatorSet createUnfocusVideoAnim(CategoryVideoHolder holder, float ratioX, float ratioY) {
        Animator scaleAnim = createScaleAnimator(holder.itemView, ratioX, ratioY);
        Animator elevationAnim = createTransZAnimator(holder.itemView, NORMAL_TRANS_Z);
        Animator titleAlphaAnim = createAlphaAnim(holder.getTitle(), 0.5f, 0.0f);
        AnimatorSet set = new AnimatorSet();
        set.setDuration(BASE_OUT_TIME);
        set.playTogether(scaleAnim, elevationAnim, titleAlphaAnim);
        return set;
    }

    public static AnimatorSet createFocusOnOtherAnim(CategoryOtherHolder holder, float ratio) {
        Animator scaleAnim = createScaleAnimator(holder.itemView, ratio, ratio);
        AnimatorSet set = new AnimatorSet();
        set.setDuration(BASE_IN_TIME);
        set.playTogether(scaleAnim);
        return set;
    }

    public static AnimatorSet createUnfocusOtherAnim(CategoryOtherHolder holder, float ratio) {
        Animator scaleAnim = createScaleAnimator(holder.itemView, ratio, ratio);
        AnimatorSet set = new AnimatorSet();
        set.setDuration(BASE_IN_TIME);
        set.playTogether(scaleAnim);
