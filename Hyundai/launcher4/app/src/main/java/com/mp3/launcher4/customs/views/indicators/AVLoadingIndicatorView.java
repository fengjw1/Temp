package com.mp3.launcher4.customs.views.indicators;
...
public class AVLoadingIndicatorView extends View {
    ...
    private static final int MSG_REDRAWN_START = 0;
    private static final int MSG_REDRAWN_STOP = 1;
    int mMinWidth;
    int mMaxWidth;
    int mMinHeight;
    int mMaxHeight;
    ...
    private HandlerThread mRedrawnThread;
    private RedrawnHandler mRedrawnHandler;
    ...
    private void initRedrawnHandler() {
        if (mRedrawnThread != null && !mRedrawnThread.isInterrupted()) {
            return;
        }
        mRedrawnThread = new HandlerThread(getClass().getSimpleName() + "Thread");
        mRedrawnThread.start();
        mRedrawnHandler = new RedrawnHandler(this, mRedrawnThread.getLooper());
    }
     ...
     */
    public void setIndicator(@Nullable String indicatorName) {
        if (TextUtils.isEmpty(indicatorName)) {
            return;
        }
        StringBuilder drawableClassName = new StringBuilder();
        final String dot = ".";
        if (!indicatorName.contains(dot)) {
    ...
    void startAnimation() {
        ...
        mRedrawnHandler.sendEmptyMessage(MSG_REDRAWN_START);
    }
    ...
    void stopAnimation() {
        ...
        mRedrawnHandler.sendEmptyMessage(MSG_REDRAWN_STOP);
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        removeCallbacks();
        if (mRedrawnThread != null) {
            mRedrawnThread.quitSafely();
        }
    }

    @Override
    public void invalidateDrawable(@NonNull Drawable dr) {
        // todo:按照原作者的意图是会走以下逻辑，
        // todo:但是在当前view依赖的Activity onPause()再onResume()刷新失效，未查明原因，故先取消之.
...
//        }
    ...
    private final static class RedrawnHandler extends Handler {

        private WeakReference<AVLoadingIndicatorView> mReference;

        RedrawnHandler(AVLoadingIndicatorView view, Looper looper) {
            super(looper);
        ...
        public void handleMessage(Message msg) {
            ...
            int what = msg.what;
            if (what == MSG_REDRAWN_START) {
                view.postInvalidate();
                sendEmptyMessageDelayed(MSG_REDRAWN_START, 10);
            } else {
                removeMessages(MSG_REDRAWN_START);
            }
        }
    }
